// src/translations.js
// ITALIAN TRANSLATIONS
const translations = {
  it: {
    welcome: "Ciao! Io sono Simone Arena. Benvenuto nel mio sito web! Clicca sulle sezioni in alto per sapere di più su di me",
    aboutMeTitle: "Me, in poche parole",
    aboutMeText:
    `
          <p>
            Ciao! Mi chiamo Simone Arena e sono nato a Catania il 22 Aprile del 1995. Ho studiato al <strong>liceo classico Mario Cutelli</strong> di Catania e poi presso <strong>l'Università di Catania</strong> dove ho conseguito il titolo di <strong>Ingegnere Informatico</strong> (laurea di primo livello). Nel mio percorso ho sempre cercato di coniugare gli studi umanistici e classici a quelli ingegneristici e scientifici. Nella mia tesi universitaria ho proprio cercato di esplorare entrambi i campi.

            <br />Sono inoltre un appassionato sviluppatore in <strong>Python</strong> e del framework <strong>React</strong> per sviluppo web, con buone conoscenze dei linguaggi di programmazione come C, C#, C++, Java, Javascript, PHP, HTML, CSS e MySQL.
            Amo sviluppare applicazioni funzionali e interattive, con design e interfacce originali.
          </p>

    <p>
            Mi piace moltissimo viaggiare e ho visitato molte città d'Europa fra cui <strong>Nizza</strong>, <strong>Cannes</strong> e il principato di <strong>Monaco</strong> durante le medie. Poi <strong>Parigi</strong> al liceo. Dopo il diploma ho frequentato il college <strong>Haileybury</strong> di Londra per due settimane, dove ho ricevuto un attestato di lingua inglese. In seguito sono tornato a <strong>Londra</strong> una seconda volta in visita. Sono anche andato a <strong>Berlino</strong> e <strong>Monaco</strong> di Baviera (dove ho visitato il castello di Neuschwanstein), poi <strong>Barcellona</strong> e <strong>Salisburgo</strong>. In Italia ho visitato molte città fra cui <strong>Milano</strong>, <strong>Roma</strong>, <strong>Torino</strong>, <strong>Palermo</strong>, <strong>Bologna</strong>, <strong>Venezia</strong>, <strong>Genova</strong>, <strong>Padova</strong> e <strong>Firenze</strong>.
            <br /><br /> Parlo <strong>Inglese</strong> ad un livello intermedio (B1) e conosco il <strong>Francese</strong> a livello elementare (A1).
            <br /><br />Durante il tempo libero mi piace molto leggere. Leggo soprattutto narrativa, classici, sci-fi, romanzi e divulgazione scientifica. I libri che ho nel cuore sono <strong>Il nome della rosa</strong> di Umberto Eco, <strong>Fiori per Algernon</strong> di Daniel Keyes, la saga di <strong>Foundation</strong> di Isaac Asimov e de <strong>Il Signore degli Anelli</strong> e il <strong>Silmarillion</strong> di J.R.R. Tolkien.
            <br /><br />Oltre a leggere, mi piace molto allenare la mente con gli scacchi. Non sono molto bravo, ma mi esercito ogni giorno.
            <br /><br />
            Per finire amo andare al cinema, guardare film e serie tv, cucinare, giocare ai videogiochi e fare escursioni una volta ogni tanto.
            <br />
          </p>`,
    backToProjects: "← Torna ai progetti",
    project1: {
      shortTitle: "Progetto IA Educativa",
      shortDesc: "Implementazione di un Agente Langchain, tramite Python e framework Langchain, per supportare lo studio e l'apprendimento",
      clickDetails: "Clicca per vedere i dettagli",
      fullTitle: "Progetto Tesi Uni: Un Agente Langchain per supportare lo studio e l'apprendimento",
      fullDesc: `
        Il progetto riguarda la realizzazione di un'applicazione, sviluppata attraverso il linguaggio Python, 
        che permette ad un qualsiasi utente di interagire con un modello di Intelligenza Artificiale (LLAMA e Chat-GPT). 
        <br /><br />
        Una volta caricato un PDF, l'utente può interrogare l'IA discutendo sul contenuto del documento.
        L'obiettivo è mostrare come l'IA integri le informazioni di pretraining con nuove conoscenze aggiornate ed affidabili.
      `,
      githubTitle: "Codice sorgente su GitHub",
      githubText: "Troverai due implementazioni: una usa LLAMA e l'altra Chat GPT 3.5 Turbo di OpenAI."
    },

    project2: {
      shortTitle: "Progetto IA OFFLINE",
      shortDesc: "Un piccolo cervello elettronico che non ha bisogno di connessione internet",
      clickDetails: "Clicca per vedere i dettagli",
      fullTitle: "Progetto IA OFFLINE: Un cervello elettronico senza internet",
      fullDesc: `
        Questo progetto nasce dal desiderio di creare un'applicazione IA capace di funzionare completamente offline, 
        sfruttando modelli pre-addestrati localmente. 
        <br /><br />
        È stato sviluppato con Python e LLAMA 3.2-3B Instruct. Utile in ambienti isolati o senza connessione Internet.
      `,
      githubTitle: "Codice su GitHub",
      githubText: "Qui trovi il codice del chatbot offline e le istruzioni per eseguire il modello IA in locale:"
    },

    project3: {
      shortTitle: "App Gestione Clienti con IA",
      shortDesc: "Un app desktop realizzata con Python per la gestione dei clienti e dei file integrata con l'IA LLAMA",
      clickDetails: "Clicca per vedere i dettagli",
      fullTitle: "App Gestione Clienti e File integrata con LLAMA IA",
      fullDesc: `Questo progetto nasce dal problema di dover gestire numerosi clienti e i relativi file e documenti. L'app è stata realizzata con <strong>Python</strong> e 
                 permette di registrare i clienti nell'app tramite nome, cognome e codice fiscale (con relativo controllo di codice fiscale corretto)
                 e offre all'utente la possibilità di gestirne i file di vari formati, PDF, JPEG, PNG eccetera. L'integrazione con l'IA LLAMA permette inoltre di 
                 recuperare le informazioni principali dai file dei clienti. 
      `,
      githubTitle: "Codice sorgente su GitHub",
      githubText: "Puoi trovare il codice completo su GitHub."
    },

    hobbies: {
      travelTitle: "✈️ Viaggiare",
      travelText: "Amo esplorare nuove città, culture e paesaggi, soprattutto in Europa.",
      chessTitle: "♟️ Giocare a Scacchi",
      chessText: "Mi alleno ogni giorno e adoro la sfida mentale che offrono gli scacchi.",
      readingTitle: "📚 Leggere",
      readingText: "Mi piacciono i romanzi classici, fantascientifici e la divulgazione scientifica.",
      gamingTitle: "🎮 Videogiochi",
      gamingText: "Gioco per rilassarmi e immergermi in mondi fantastici e stimolanti.",
      cinemaTitle: "🎬 Cinema & Serie TV",
      cinemaText: "Adoro guardare film di ogni genere e seguire serie coinvolgenti."
    }
  },



  // _________________________________________________________________________________
  // ENGLISH TRANSLATIONS
  en: {
    welcome: "Hello! I'm Simone Arena. Welcome to my website! Click on the sections above to find out more about me",
    aboutMeTitle: "Me, in a nutshell",
    aboutMeText:
    `
    <p>
Hello! My name is Simone Arena and I was born in Catania on 22 April 1995. I studied at the <strong>Mario Cutelli Classical High School</strong> in Catania and then at the <strong>University of Catania</strong>, where I obtained a degree in <strong>Computer Engineering</strong> (first level degree). Throughout my studies, I have always tried to combine humanities and classics with engineering and science. In my university thesis, I sought to explore both fields.

            <br />I am also a passionate developer in Python and the React framework for web development, with a good knowledge of programming languages such as C, C#, C++, Java, JavaScript, PHP, HTML, CSS and MySQL.
            I love developing functional and interactive applications with original designs and interfaces.
          </p>

    <p>
            I love travelling and visited many cities in Europe, including Nice, Cannes and the Principality of Monaco during secondary school. Then Paris during sixth form. After graduating, I attended Haileybury College in London for two weeks, where I received an English language certificate. Later, I returned to <strong>London</strong> for a second visit. I also went to <strong>Berlin</strong> and <strong>Munich</strong> in Bavaria (where I visited Neuschwanstein Castle), then <strong>Barcelona</strong> and <strong>Salzburg</strong>. In Italy, I visited many cities including Milan, Rome, Turin, Palermo, Bologna, Venice, Genoa, Padua and Florence.
            <br /><br /> I speak <strong>English</strong> at an intermediate level (B1) and have a basic knowledge of <strong>French</strong> (A1).
<br /><br />In my free time, I really enjoy reading. I mainly read fiction, classics, sci-fi, novels and popular science books. My favourite books are <strong>The Name of the Rose</strong> by Umberto Eco, <strong>Flowers for Algernon</strong> by Daniel Keyes, the <strong>Foundation</strong> series by Isaac Asimov, and <strong>The Lord of the Rings</strong> and <strong>The Silmarillion</strong> by J.R.R. Tolkien.
            <br /><br />In addition to reading, I really enjoy training my mind with chess. I'm not very good at it, but I practise every day.
<br /><br />
Finally, I love going to the cinema, watching films and TV series, cooking, playing video games and going on excursions every now and then.
<br />
</p>`,

    backToProjects: "← Back to projects",
    project1: {
      shortTitle: "Educational AI Project",
      shortDesc: "Implementation of a Langchain Agent, using Python and the Langchain framework, to support study and learning",
      clickDetails: "Click to view details",
      fullTitle: "University Thesis Project: A Langchain Agent to support study and learning",
      fullDesc: `
        The project consists of developing an application in Python 
        that allows any user to interact with an AI model (LLAMA and Chat-GPT). 
        <br /><br />
        Once a PDF is uploaded, the user can ask the AI questions about its content.
        The goal is to show how AI combines pretraining knowledge with new, reliable and updated information.
      `,
      githubTitle: "Source code on GitHub",
      githubText: "You will find two implementations: one with LLAMA and the other with Chat GPT 3.5 Turbo by OpenAI."
    },

    project2: {
      shortTitle: "OFFLINE AI Project",
      shortDesc: "A small electronic brain with no internet required",
      clickDetails: "Click to view details",
      fullTitle: "OFFLINE AI Project: An electronic brain without internet",
      fullDesc: `
        This project was born from the idea of creating an AI application 
        capable of working completely offline using locally pre-trained models. 
        <br /><br />
        It was developed with Python and LLAMA 3.2-3B Instruct. 
        Useful in isolated environments or where Internet access is not available.
      `,
      githubTitle: "Code on GitHub",
      githubText: "Here you can find the offline chatbot code and instructions to run the AI model locally:"
    },
    project3: {
      shortTitle: "Customer Management App with AI",
      shortDesc: "A desktop application developed with Python for customer and file management, integrated with LLAMA AI.",
      clickDetails: "Click to see details",
      fullTitle: "Customer and File Management App integrated with LLAMA IA",
      fullDesc: `This project arose from the problem of having to manage numerous customers and their files and documents. The app was developed using <strong>Python</strong> and 
                 allows customers to be registered in the app by name, surname and tax code (with verification of the correct tax code)
                 and offers users the possibility to manage files in various formats, such as PDF, JPEG, PNG, etc. Integration with LLAMA AI also allows 
                 key information to be retrieved from customer files.`,
      githubTitle: "Source code on GitHub",
      githubText: "You can find the full code on GitHub."
    },

    hobbies: {
      travelTitle: "✈️ Traveling",
      travelText: "I love exploring new cities, cultures, and landscapes, especially in Europe.",
      chessTitle: "♟️ Playing Chess",
      chessText: "I practice every day and love the mental challenge chess offers.",
      readingTitle: "📚 Reading",
      readingText: "I enjoy classic novels, science fiction, and popular science books.",
      gamingTitle: "🎮 Video Games",
      gamingText: "I play to relax and immerse myself in fantastic and stimulating worlds.",
      cinemaTitle: "🎬 Movies & TV Series",
      cinemaText: "I love watching films of all kinds and following engaging series."
    }
  }
};

export default translations;